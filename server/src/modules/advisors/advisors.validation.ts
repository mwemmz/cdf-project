...
export const ADVISOR_SPECIALTIES = ['business_plans', 'feasibility', 'accounting'] as const;

export const createAdvisorProfileSchema = z.object({
  specialty: z.enum(ADVISOR_SPECIALTIES, {
    errorMap: () => ({ message: 'Specialty must be one of: business_plans, feasibility, accounting' }),
  }),
