...
4. Open **fundpath-api** → **Environment** and set:
   ...
   `prisma migrate deploy`**, so tables are created on Neon automatically. The seed is
   intentionally not run automatically — see "Seeding the production DB" below.
...
- **Seeding the production DB:** run the seed once from your machine against Neon by setting
  your Neon URL in `server/.env` and running `pnpm seed` (uses `server/.env`). Or add a
  one-off backend build/shell. Test logins are in the seed section above.
